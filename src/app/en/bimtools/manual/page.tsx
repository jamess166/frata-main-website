...
const suiteDescriptions: Record<string, string> = {
  "architecture-create": "Automation to turn architectural room data into modeled floor elements with less manual setup.",
  ...
  "structure-create": "Automation for reinforcement creation and structural production.",
  "structure-modify": "Utilities to edit, synchronize and visualize reinforcement workflows.",
};
        ...
        <div className="container mx-auto px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
          <div className="max-w-4xl">
            ...
            <h1 className="mt-6 font-headline text-4xl font-bold tracking-tight sm:text-5xl">
              Documentation for every BIMtools add-in in one place.
            </h1>
            <p className="mt-6 max-w-3xl text-lg leading-8 text-muted-foreground">
              Open each tool separately to review features, scope and usage procedures based on the real markdown
              documentation shipped with your add-ins.
            </p>
          </div>
        </div>
      </section>
            ...
            <div key={suite.id}>
              <div className="flex flex-col gap-4 border-b pb-6 lg:flex-row lg:items-end lg:justify-between">
                <div className="max-w-2xl">
                  ...
                  <p className="mt-4 text-muted-foreground">{suiteDescriptions[suite.id] ?? suite.description}</p>
                </div>
                <p className="text-sm text-muted-foreground">{suite.manuals.length} manuals available</p>
              </div>
                  ...
                  <Link key={manual.slug} href={`/en/bimtools/manual/${manual.slug}`} className="block h-full">
                    <Card className="h-full border-border/70 bg-card/70 transition-colors hover:border-primary/40">
                      ...
                      <CardContent className="flex h-full flex-col">
                        <p className="flex-grow text-sm leading-7 text-muted-foreground">{manual.excerpt.en}</p>
                      </CardContent>
                    </Card>
                  </Link>
                ))}
